<script lang="ts">
    ...
    let {label, id, name, group = $bindable(), value, disabled = false}: {label: string, id: string, name: string, group: string, value: string, disabled?: boolean} = $props();
</script>

<input bind:group={group} {name} {id} type="radio" class="peer hidden" value={value} disabled={disabled}>
